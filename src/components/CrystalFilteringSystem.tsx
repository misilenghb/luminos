...
import { crystalTypeMapping, type CrystalData } from '@/lib/crystal-options';
...
import { Filter, XCircle, Gem } from 'lucide-react';
                  ...
                  {filteredCrystals.map(crystal => (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground mt-1 line-clamp-2">
                        {crystal.healingProperties?.slice(0, 2).map((propKey: string) => t(`options.crystalProperties.${propKey}`)).join(', ') || t('common.noHealingProperties')}
                      </span>
                    </Button>
                  ))}
                </div>
