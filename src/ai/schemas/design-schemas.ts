...
export const BeadSchema = z.object({
  ...
  type: z.string().optional(), // e.g., 'spacer', 'crystal'
  role: z.string().optional(), // e.g., 'focal', 'accent'
  position: z.object({
    x: z.number(),
    y: z.number(),
  }).optional(),
...
export const DesignSuggestionsInputSchema = z.object({
  ...
  userIntent: z
    .string()
    .optional()
    .describe("The user's purpose for the design, e.g., gift, personal talisman."),
  imageStyle: z
    .string()
    .optional()
    .describe('The desired artistic style for the design image preview, e.g., colored pencil, oil painting, photo.'),
  applicationPlatform: z
    .string()
    .optional()
    .describe('The target image generation platform, e.g., "Midjourney", "DALL-E 3".'),
  aspectRatio: z
    .string()
    .optional()
    .describe('The desired aspect ratio for the image, e.g., "16:9", "1:1".'),
  mainStones: z
    .string()
  ...
  metalType: z.string().optional().describe('The primary metal for the design, e.g., Gold, Silver, Rose Gold.'),
...
export const DesignSchemeSchema = z.object({
  ...
  imageGenerationPrompt: z.string().describe("A detailed, ready-to-use prompt in English for generating a photorealistic image of this specific design scheme, tailored for the selected generation platform and aspect ratio."),
});
